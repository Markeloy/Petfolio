-- Run as the database administrator against the existing schema.
-- Fixtures and all dose/audit writes are rolled back, including test auth users.
BEGIN;
DO $$
DECLARE
  ...
  stamp timestamptz := date_trunc('minute', now()); first_mark record; second_mark record; skip_mark record;
BEGIN
  ...
  SELECT * INTO skip_mark FROM public.record_medication_dose(sched,stamp+interval '1 minute','skipped');
