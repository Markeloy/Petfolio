...
export async function feedingReminders(client:SupabaseClient<Database>,petIds:string[],now=new Date(),locale='ru-RU') {
  ...
  try {
    await Promise.all(petIds.map(async petId=>{
      const [plans,logs]=await Promise.all([
        allPages((from,to)=>client.from('feeding_plans').select('*').eq('pet_id',petId).is('archived_at',null).order('id').range(from,to)),
        allPages((from,to)=>client.from('feeding_logs').select('plan_id,planned_on').eq('pet_id',petId).gt('scheduled_for',now.toISOString()).order('id').range(from,to)),
      ]);
