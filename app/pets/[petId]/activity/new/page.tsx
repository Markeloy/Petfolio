...
export default async function NewActivity({params}:{params:Promise<{petId:string}>}) {
  ...
  return <><Link href={`/pets/${petId}/activity`}>{t("← Активность")}</Link><h1>{t("Новая активность")}</h1><p>{pet.name} · {timezone}</p>{canEdit?<ActivityForm mode="create" petId={petId} id={randomUUID()} initialTime={initialTime}/>:<p>{t("У вас нет права изменять активность.")}</p>}</>;
}
