...
export function HealthForm({petId,id,event,today,repeat,initialKind}:{petId:string;id:string;event?:HealthEvent;today:string;repeat?:HealthEvent;initialKind?:HealthKind}) {
  ...
  return <form action={action} className="healthForm wizardFields"><AnalyticsFormFields/>
    ...
    <div className="splitFields">
      ...
      <label><span>{t("Состояние")}</span><select name="status" {...field('status')}>{Object.entries(healthStatuses).filter(([key])=>!repeat||key!=='cancelled').map(([key,label])=><option value={key} key={key}>{values.kind==='symptom'&&key==='completed'?t("Наблюдение записано"):t(label)}</option>)}</select></label>
    </div>
