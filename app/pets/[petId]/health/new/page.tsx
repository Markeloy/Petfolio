...
export default async function NewHealth({params,searchParams}:{params:Promise<{petId:string}>;searchParams:Promise<{repeat?:string}>}) {
  ...
  return <main className="detailShell healthShell"><HealthHeader petId={petId} name={pet.name} title={repeat?'Повторное событие':'Новое событие'} back={`/pets/${petId}/health`}/><HealthForm petId={petId} id={randomUUID()} repeat={repeat} today={localDate(new Date(),timezone)}/></main>;
}
