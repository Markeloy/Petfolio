...
export default async function NewWeight({params}:{params:Promise<{petId:string}>}) {
  ...
  return <main className="detailShell healthShell"><HealthHeader petId={petId} name={pet.name} title="Новое измерение" back={`/pets/${petId}/health/weight`}/><WeightForm petId={petId} id={randomUUID()} datetime={datetime} timezone={timezone}/></main>;
}
