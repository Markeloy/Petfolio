...
export default async function WeightDetail({params}:{params:Promise<{petId:string;recordId:string}>}) {
  ...
  return <main className="detailShell healthShell"><HealthHeader petId={petId} name={pet.name} title={record.archived_at?'Измерение в архиве':'Измерение веса'} back={`/pets/${petId}/health/weight`}/>
    {record.archived_at?<section className="formSectionCard"><h2>{record.weight_kg.toLocaleString('ru-RU')} кг</h2><p>{formatMoment(record.measured_at,timezone)}</p><p className="healthNotes">{record.notes}</p></section>:<WeightForm petId={petId} id={recordId} record={record} datetime={datetime} timezone={timezone}/>}
