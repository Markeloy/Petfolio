...
export default async function EditHealth({params}:{params:Promise<{petId:string;eventId:string}>}) {
  ...
  return <main className="detailShell healthShell"><HealthHeader petId={petId} name={pet.name} title={t("Изменить событие")} back={`/pets/${petId}/health/events/${eventId}`}/><HealthForm petId={petId} id={eventId} event={event} today={localDate(new Date(),timezone)}/></main>;
}
