...
export default async function NewDocument({params,searchParams}:{params:Promise<{petId:string}>;searchParams:Promise<{resume?:string}>}) {
  ...
  return <><Link href={`/pets/${petId}/documents`}>← Документы</Link><h1>{resume?'Продолжить загрузку':'Новый документ'}</h1><p>{pet.name}</p>{canEdit?<DocumentForm mode="upload" petId={petId} documentId={resume??randomUUID()} document={draft?.data??undefined} key={resume??'new'}/>:<p>У вас нет права загружать документы.</p>}</>;
}
