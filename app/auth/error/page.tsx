...
export default async function AuthErrorPage({ searchParams }: { searchParams: Promise<{ reason?: string }> }) {
  ...
  return <main className="authShell"><section className="authCard compactAuthCard"><p className="authBrand">Petfolio</p><div className="authIntro"><span className="authPaw">🐾</span><h1>Что-то пошло не так</h1><p>{message}</p></div><Link className="primaryAction linkAction" href="/login">Вернуться ко входу</Link></section></main>;
}
