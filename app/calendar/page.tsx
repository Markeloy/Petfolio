...
export default async function CalendarPage({searchParams}: {searchParams: Promise<{date?: string; pet?: string}>}) {
  ...
  return <main className="appShell"><RefreshOnFocus/><div className="content calendarContent">
    ...
    <section aria-labelledby="day-title"><h2 id="day-title">{dateLabel(day)}{day === today ? ' · Сегодня' : ''}</h2>
      <p className="calendarNote">Время приёмов и кормлений: {timezone}. События здоровья — без указания времени.</p>
      {day < today && <p className="calendarNote">За прошлые дни показаны сохранённые отметки лекарств, кормлений и записи здоровья. Отсутствие отметки не означает пропуск.</p>}
      ...
      <p className="calendarNote">Откройте запись, чтобы посмотреть подробности. Лекарство или кормление можно отметить на его экране в день события по расписанию.</p>
    </section>
